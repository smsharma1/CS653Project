...
pureLitElem (cs,vs) ocs = takeDecision (cs,vs) (findFirstPureLit (cs,vs)) ocs

takeDecision (cs, vs) decV ocs | decV==0 = dpll (cs,vs)
                            | otherwise = (\(sat, ass) -> (sat, (decV:ass))) (pureLitElem ((clauseElem cs decV), L.delete (abs decV) vs) ocs)


decision_up (cs,vs) ocs | elem [] cs    =   (False, [])
                    | vs == []       =  (True, [])
                    | otherwise     =  unitPropogate (cs,vs) (findsinglevar cs) ocs
...
unitPropogate (cs,vs) decV ocs | decV == 0 =   pureLitElem (cs,vs) ocs
                            | otherwise =  findConflictClause ((\(sat, ass) -> (sat, (decV:ass))) (decision_up (clauseElem cs decV, L.delete (abs decV) vs) ocs)) ocs

findConflictClause (sat1, ass1) ocs = case sat1 of
                                        True -> (sat1, ass1)
                                        False -> 
                                            -- enumerate all original clauses which have become unsat
                                            -- resolve them to get conflict clause

-- returns at first True due to lazy evaluation
startdpll (cs,vs) ocs  | elem [] cs    =   (False, [])
                    | vs == []        =   (True, [])
                    | otherwise      =  branchLiteral decV (decision_up (clauseElem cs decV, L.delete (abs decV) vs) ocs) (decision_up (clauseElem cs (-1*decV), L.delete (abs decV) vs) ocs)
